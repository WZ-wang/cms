    ...
    <div class="content">
      <ul>
        <li v-for="(item,index) in programData" :key="index" >
          <div class="wrapper">
            <img @click="programDetail(item)" src="../../../assets/program.jpg" alt />
            <div class="pInfo">
              <h3>{{item.name}}</h3>
              <div class="date">
                <p>收藏时间:{{item.dates}}</p>
...
export default {
  ...
  methods: {
    ...
    getData(){
      ...
      this.$axios.post("/api/note/collectList",programDetail).then(res => {
        console.log(res)
    ...
    del(item){
      ...
      collectionData.append("id", item.id);
      collectionData.append("phone", item.phone);
